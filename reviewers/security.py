...
Responsibility:
    Evaluate the diff for security risk — authentication, authorization,
    secrets management, token handling, input validation, logging of
    sensitive data, replay protection, cryptographic practices, and other
    secure coding concerns.
...
Sprint 12 note: review() makes a REAL Cursor SDK call, following the exact
same pattern reviewers/architecture.py established (Sprint 6/7 direct
in-process integration, Sprint 8 structured decision/confidence/findings/
recommendations/overall_assessment output rendered into one markdown
report). This module is intentionally a close copy of architecture.py —
same helper shapes, same JSON contract, same rendering, same error
handling — with only the persona (prompts/security.md) and reviewer name
different, per this sprint's explicit "keep it as similar as practical"
instruction.
...
error in the summary, same convention as reviewers/architecture.py.

reviewers/cab.py needed NO changes to consume this: it already reads
ReviewResult.decision/.verdict/.overall_assessment/.recommendations
generically for any reviewer (that's how it already told Architecture
apart from the still-hardcoded QA/Operations placeholders). Once this
module populates `decision`, CAB automatically treats Security as a real
reviewer instead of a "(Placeholder)" one.
"""
...
_REVIEWER_NAME = "Security"
_PROMPT_TEMPLATE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "prompts", "security.md")
...
def _render_report_markdown(
    ...
    approach as reviewers/architecture.py's _render_report_markdown()
    (Sprint 8), since orchestrator/pipeline.py yields this reviewer's full
    ReviewResult object (Sprint 11) but ui/ still just displays
    `.summary` as one markdown blob.
