...
│   ├── security.py            # review(submission) -> ReviewResult   [REAL Cursor SDK call, in-process, as of Sprint 12]
│   ├── qa.py                  # review(submission) -> ReviewResult   [REAL Cursor SDK call, in-process, as of Sprint 13]
...
│   ├── security.md            # REAL content as of Sprint 12 — loaded by reviewers/security.py
│   ├── qa.md                  # REAL content as of Sprint 13 — loaded by reviewers/qa.py
...
| **Sprint 12** | Replace the placeholder Security reviewer with a real Cursor SDK reviewer | Authored `prompts/security.md` for real (was an empty placeholder) — same JSON schema as `architecture.md` (`decision`/`confidence`/`summary`/`findings`/`recommendations`/`overall_assessment`), persona focused on the 8 areas requested (authN, authZ, secrets, token handling, input validation, sensitive-data logging, replay protection, crypto/secure-coding), "out of scope" pointing at Architecture/QA/Operations. `reviewers/security.py` is an intentional close copy of `reviewers/architecture.py`'s Sprint 6/7/8 shape — same `_load_persona_prompt()`/`_build_prompt()`/`_group_findings_by_severity()`/`_render_report_markdown()`/`_blocked_result()`/`_parse_agent_response()`/`review()`, same in-process `Agent.prompt(...)` call, same never-raises/`decision="BLOCK"`-on-failure safety net — with only `_REVIEWER_NAME`, the prompt path, and docstrings changed. **`reviewers/cab.py` needed zero code changes** — verified live: it already reads `ReviewResult.decision`/`.verdict`/`.overall_assessment` generically for any reviewer (that's how it already told Architecture apart from the still-hardcoded QA/Operations placeholders via `result.decision is None`), so once Security populated `.decision` for real, CAB's "Reviewer Summary" automatically dropped the `_(Placeholder)_` tag and folded Security's actual `NEEDS_CHANGES` into its own decision (HIGH risk) with no code change. `ui/components.py::decision_badge()` also needed no changes — it picked up Security's real badge the same way (its expander defaulted to collapsed for Security at the time, per Sprint 11's `_DEFAULT_EXPANDED`, since that map wasn't in this sprint's scope; the user separately requested that one-line flip to `True` right after this sprint landed — done, no other changes). Verified with a mocked no-credentials failure path plus a real, live full-pipeline round trip against `demo/auth-service.diff`: caught the diff's actual vulnerabilities (refresh token logged in cleartext, shared access/refresh signing secret, no revocation/rotation despite the PR describing it) as HIGH/MEDIUM findings with 88% confidence. `orchestrator/pipeline.py`, `models/review_models.py`, `ui/`, `reviewers/architecture.py`, `reviewers/cab.py`, `qa.py`, and `operations.py` are byte-for-byte unchanged. |
| **Sprint 13** | Replace the placeholder QA reviewer with a real Cursor SDK reviewer | Authored `prompts/qa.md` for real (was an empty placeholder) — same JSON schema as `architecture.md`/`security.md` (`decision`/`confidence`/`summary`/`findings`/`recommendations`/`overall_assessment`), persona focused on testability/test coverage/verifiability (proportional coverage, edge cases implied by the PR, regression risk, pre-deployment verifiability), "out of scope" pointing at Architecture/Security/Operations. `reviewers/qa.py` is an intentional close copy of `reviewers/architecture.py`'s/`reviewers/security.py`'s shape — same `_load_persona_prompt()`/`_build_prompt()`/`_group_findings_by_severity()`/`_render_report_markdown()`/`_blocked_result()`/`_parse_agent_response()`/`review()`, same in-process `Agent.prompt(...)` call, same never-raises/`decision="BLOCK"`-on-failure safety net — with only `_REVIEWER_NAME`, the prompt path, and docstrings changed. **No orchestrator change was needed at all** — QA has been registered in `orchestrator/pipeline.py`'s `_DIFF_REVIEWERS` tuple since Sprint 3's placeholder scaffold, in the correct position; only its `review()` body changed. **`reviewers/cab.py` needed zero code changes**, same reasoning as Sprint 12: it reads `ReviewResult.decision` generically, so once QA populated it for real, CAB's "Reviewer Summary" automatically dropped the `_(Placeholder)_` tag for QA and folded its real `NEEDS_CHANGES` into an overall `NEEDS_CHANGES`/HIGH-risk decision alongside Architecture and Security. Verified with a mocked no-credentials failure path plus a real, live full-pipeline round trip against `demo/auth-service.diff`: correctly flagged the diff's new refresh endpoint as having zero automated tests and unhandled `jwt.verify` failure paths, at 88% confidence. `orchestrator/pipeline.py`, `models/review_models.py`, `ui/`, `reviewers/architecture.py`, `reviewers/security.py`, `reviewers/cab.py`, and `operations.py` are byte-for-byte unchanged. |
...
- `reviewers/security.py` — **real as of Sprint 12.** Intentional close copy of `architecture.py`'s Sprint 6/7/8 shape (see the Sprint 12 row in §4) — same in-process call, same JSON contract, same rendering, same failure handling. Verified live against `demo/auth-service.diff`: found the diff's actual token-logging/shared-secret/no-revocation issues as HIGH/MEDIUM findings. `reviewers/cab.py` required zero changes to pick this up as a real reviewer instead of a placeholder.
- `reviewers/qa.py` — **real as of Sprint 13.** Same close-copy pattern as Security (see the Sprint 13 row in §4). Verified live against `demo/auth-service.diff`: found the diff's actual missing-test-coverage and unhandled-`jwt.verify`-failure issues as HIGH/MEDIUM findings. `reviewers/cab.py` and `orchestrator/pipeline.py` required zero changes — QA was already registered in `_DIFF_REVIEWERS` since Sprint 3.

**Explicitly placeholder / not real yet:**
- `operations.py` — `review()` body still returns **hardcoded** results; untouched since Sprint 3.
- No Cursor SDK import anywhere in `orchestrator/`, `ui/`, `models/`, `config.py`'s logic, or `reviewers/operations.py` — `reviewers/architecture.py`, `reviewers/security.py`, `reviewers/qa.py`, and `reviewers/cab.py` are the only four that import `cursor_sdk`.
- `prompts/operations.md` is still an empty template placeholder (`architecture.md` real as of Sprint 6/expanded Sprint 8; `cab.md` real as of Sprint 9; `security.md` real as of Sprint 12; `qa.md` real as of Sprint 13).
...
- `Finding` objects are populated for real for Architecture (Sprint 8), Security (Sprint 12), and QA (Sprint 13); still always `[]` for `operations.py`. CAB has no `Finding`-shaped output of its own (out of scope for its schema).
- `ui/components.py::_DEFAULT_EXPANDED["QA"]` is still `False` (collapsed by default) even though QA is real as of Sprint 13 — that map is Sprint 11's UI-layer decision and wasn't in this sprint's scope. (`_DEFAULT_EXPANDED["Security"]` got this same treatment as a small, explicit, separately-requested follow-up right after Sprint 12 — flipped to `True` — so this is a known, precedented follow-up if wanted for QA too.)
...
- `reviewers/operations.py` — same treatment as `architecture.py`/`security.py`/`qa.py`/`cab.py`: load the matching `prompts/operations.md` template, call `cursor_sdk.Agent.prompt(...)` directly (no bridge needed anymore), parse structured response into `ReviewResult`. Last remaining hardcoded reviewer.
- Author real prompt content in `prompts/operations.md` (currently an empty HTML-comment placeholder; `architecture.md`, `cab.md`, `security.md`, and `qa.md` are done).
- Convert the diff-reviewer fan-out from sequential to concurrent (Cursor SDK async client — `AsyncAgent`/`AsyncClient`), per `ARCHITECTURE.md` §2 and §5. This is simpler post-Sprint-7 than it would have been with the old subprocess bridge — it's now "N concurrent in-process `Agent.prompt` calls" rather than "N concurrent subprocesses."
- Define and implement partial-failure policy (can CAB proceed with 3/4 reviews being real vs. hardcoded, or one being a `block`-on-error placeholder like Architecture/Security/QA can now produce? how are `CursorAgentError` vs. `result.status == "error"` vs. parse failures surfaced beyond each reviewer's current "collapse to `block`" approach?).
...
- ~~Replace the placeholder Security reviewer with a real Cursor SDK reviewer~~ — **done in Sprint 12**, verified live against the same fixture — see the Sprint 12 row in §4.
- ~~Replace the placeholder QA reviewer with a real Cursor SDK reviewer~~ — **done in Sprint 13**, verified live against the same fixture — see the Sprint 13 row in §4.
...
- Give `operations.py` the same `decision`/`confidence`/`recommendations`/`overall_assessment` treatment once it gets a real SDK call, so all four diff-reviewer cards render consistently (and so CAB's per-reviewer summary line for it reads as more than a one-line placeholder).
- Set `ui/components.py::_DEFAULT_EXPANDED["QA"] = True` if the demo wants QA's expander open by default now that it's real, matching Architecture/Security/CAB (see §6's placeholder note — not done in Sprint 13, out of that sprint's stated scope; this is exactly the follow-up the user separately requested for Security right after Sprint 12).
- Refresh `README.md` and `PRODUCT_VISION.md` — see staleness note below.
- `ruff check .` now reports pre-existing `UP006`/`UP035`/`UP045` findings (`typing.Optional`/`List`/`Dict`/`Tuple` → `X | None`/`list`/`dict`) across most files, including some Sprint 8/9/12/13 touched — ruff started flagging these once `.python-version` (Sprint 7) gave it a Python 3.12 target to infer. This is pre-existing style debt, not a regression from any of those sprints (see §5 item 8) — a future sprint could modernize it repo-wide in one pass if asked, rather than doing it piecemeal per-file.
...
tighter, decision-first executive report. Sprint 11 polished the overall
UI (header, expanders, decision badges). Sprint 12 wired up a third real
reviewer (Security), same pattern as Architecture. Sprint 13 wired up a
fourth real reviewer (QA), same pattern again. Remaining candidates:
...
8. ~~Final UI polish~~ — **done in Sprint 11** (header, expanders, decision badges, pipeline yield enrichment; see §4's Sprint 11 row).
9. ~~Replace the placeholder Security reviewer with a real Cursor SDK reviewer~~ — **done in Sprint 12**, same pattern as Architecture; `cab.py` needed zero changes to consume it (see §4's Sprint 12 row).
10. ~~Replace the placeholder QA reviewer with a real Cursor SDK reviewer~~ — **done in Sprint 13**, same pattern as Architecture/Security; neither `cab.py` nor `orchestrator/pipeline.py` needed changes (see §4's Sprint 13 row).
11. **Repeat the pattern for Operations** (now a near-copy of `reviewers/architecture.py`'s/`security.py`'s/`qa.py`'s direct-call shape *and* its structured-output/rendering shape, with its own `prompts/operations.md`) — the last remaining hardcoded diff-reviewer. This would also make CAB's per-reviewer "Reviewer Summary" line for it richer, since it already reads `overall_assessment`/`decision` when present and only falls back to the one-line placeholder `summary` otherwise.
12. **Consider the Cursor SDK's async client** for the four diff-reviewers now that three of four are real, so they can run concurrently instead of sequentially (see the §7 TODO) — this is now a much simpler change than it would have been under the Sprint 6 bridge design, since there's no subprocess-per-call question anymore.
13. ~~Consider setting `ui/components.py::_DEFAULT_EXPANDED["Security"] = True`~~ — **done**, as a small, explicit, separately-requested follow-up right after Sprint 12. Same candidate now exists for `_DEFAULT_EXPANDED["QA"]` following Sprint 13 (see §7 TODO — small, UI-only, not done in Sprint 13 since it wasn't in that sprint's stated scope).

**Do not start Cursor SDK / LLM work *inside a reviewer* without explicit user confirmation** — every prior sprint in this project has been explicitly scoped by the user one step at a time. Sprint 6/7 explicitly authorized real SDK logic in the Architecture reviewer, Sprint 9 in the CAB reviewer, Sprint 12 in the Security reviewer, Sprint 13 in the QA reviewer; `operations.py` remains off-limits until a future sprint says otherwise.
...
- **Do not add Cursor SDK, LLM, or other AI logic to `operations.py`, `orchestrator/`, `ui/`, `models/`, or `config.py`'s existing settings** unless the user explicitly requests it in a given sprint. This has been an explicit, repeated constraint across every sprint so far. (`verify_cursor_sdk.py` (Sprint 5), `reviewers/architecture.py` + the `CURSOR_API_KEY`/`CURSOR_SDK_MODEL` settings in `config.py` (Sprint 6/7), `reviewers/cab.py` (Sprint 9), `reviewers/security.py` (Sprint 12), and `reviewers/qa.py` (Sprint 13) are the explicit, scoped exceptions so far — `architecture.py`, `security.py`, `qa.py`, and `cab.py` are the only four reviewers with real SDK logic.)
