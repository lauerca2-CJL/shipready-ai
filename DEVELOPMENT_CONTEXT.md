...
- `orchestrator/pipeline.py` still only forwards `ReviewResult.summary` / `ReleaseDecision.rationale` (plain strings) to the dashboard, not the full objects — Sprint 8/9's rich rendering works around this by rendering the whole report into that one string each (see §2 / the Sprint 8/9 rows in §4). A future sprint enriching the orchestrator's yield to pass the full objects would let `ui/` build the report layout itself instead of receiving pre-rendered markdown — noted as a real design trade-off, not an oversight.
...
in the same report style. Remaining candidates:
...
7. **Repeat the pattern for Security, QA, Operations** (each is now a near-copy of `reviewers/architecture.py`'s direct-call shape *and* its Sprint 8 structured-output/rendering shape, with its own `prompts/*.md`) — this would also make CAB's per-reviewer "Reviewer Summary" lines richer for those three, since it already reads `overall_assessment`/`decision` when present and only falls back to the one-line placeholder `summary` otherwise.
8. **Consider the Cursor SDK's async client** for the four diff-reviewers once more than one is real, so they can run concurrently instead of sequentially (see the §7 TODO) — this is now a much simpler change than it would have been under the Sprint 6 bridge design, since there's no subprocess-per-call question anymore.
9. **Consider enriching `orchestrator/pipeline.py`'s yield** to pass the full `ReviewResult`/`ReleaseDecision` (not just `.summary`/`.rationale`) to the dashboard, now that two reviewers have Sprint 8/9-style structured output — that would let `ui/` build each report's layout directly instead of every reviewer having to pre-render its own markdown. Sprints 8 and 9 both deliberately didn't do this since the orchestrator was explicitly out of scope each time; confirm with the user before touching it.
...
- **`orchestrator/pipeline.py` only forwards `ReviewResult.summary` / `ReleaseDecision.rationale`** (plain strings) to the dashboard — not the other structured fields (`decision`/`confidence`/`findings`/`recommendations`/`overall_assessment` on `ReviewResult`; `cab_decision`/`overall_risk`/`executive_summary`/`reviewer_summary`/`business_impact`/`final_recommendation` on `ReleaseDecision`). This is why Sprint 8/9 render the whole report *into* `summary`/`rationale` as one markdown string rather than changing the orchestrator's yield. Don't assume the dashboard can read those other fields directly; it can't, without an orchestrator change (see the §8 candidate for that).
