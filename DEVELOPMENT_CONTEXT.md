...
- `qa.py`, `operations.py` — `review()` bodies still return **hardcoded** results; untouched since Sprint 3.
- No Cursor SDK import anywhere in `orchestrator/`, `ui/`, `models/`, `config.py`'s logic, or `reviewers/qa.py` / `operations.py` — `reviewers/architecture.py`, `reviewers/security.py`, and `reviewers/cab.py` are the only three that import `cursor_sdk`.
- `prompts/qa.md`, `operations.md` are still empty template placeholders (`architecture.md` real as of Sprint 6/expanded Sprint 8; `cab.md` real as of Sprint 9; `security.md` real as of Sprint 12).
...
- `Finding` objects are populated for real for Architecture (Sprint 8) and Security (Sprint 12); still always `[]` for `qa.py`/`operations.py`. CAB has no `Finding`-shaped output of its own (out of scope for its schema).
- `ui/components.py::_DEFAULT_EXPANDED["Security"]` is still `False` (collapsed by default) even though Security is real as of Sprint 12 — that map was Sprint 11's UI-layer decision and wasn't in this sprint's scope; a real, disclosed follow-up if the demo wants Security expanded by default too.
...
- `reviewers/qa.py`, `operations.py` — same treatment as `architecture.py`/`security.py`/`cab.py`: load the matching `prompts/*.md` template, call `cursor_sdk.Agent.prompt(...)` directly (no bridge needed anymore), parse structured response into `ReviewResult`.
- Author real prompt content in `prompts/qa.md`, `operations.md` (currently empty HTML-comment placeholders; `architecture.md`, `cab.md`, and `security.md` are done).
...
- Define and implement partial-failure policy (can CAB proceed with 2/4 reviews being real vs. hardcoded, or one being a `block`-on-error placeholder like Architecture/Security can now produce? how are `CursorAgentError` vs. `result.status == "error"` vs. parse failures surfaced beyond each reviewer's current "collapse to `block`" approach?).
...
- Give `qa.py`/`operations.py` the same `decision`/`confidence`/`recommendations`/`overall_assessment` treatment once they get real SDK calls, so all four diff-reviewer cards render consistently (and so CAB's per-reviewer summary line for them reads as more than a one-line placeholder).
- Set `ui/components.py::_DEFAULT_EXPANDED["Security"] = True` if the demo wants Security's expander open by default now that it's real, matching Architecture/CAB (see §6's placeholder note — not done in Sprint 12, out of that sprint's stated scope).
- Refresh `README.md` and `PRODUCT_VISION.md` — see staleness note below.
- `ruff check .` now reports pre-existing `UP006`/`UP035`/`UP045` findings (`typing.Optional`/`List`/`Dict`/`Tuple` → `X | None`/`list`/`dict`) across most files, including some Sprint 8/9/12 touched — ruff started flagging these once `.python-version` (Sprint 7) gave it a Python 3.12 target to infer. This is pre-existing style debt, not a regression from any of those sprints (see §5 item 8) — a future sprint could modernize it repo-wide in one pass if asked, rather than doing it piecemeal per-file.
...
reviewer (Security), same pattern as Architecture. Remaining candidates:
...
10. **Repeat the pattern for QA, Operations** (each is now a near-copy of `reviewers/architecture.py`'s/`security.py`'s direct-call shape *and* its structured-output/rendering shape, with its own `prompts/*.md`) — this would also make CAB's per-reviewer "Reviewer Summary" lines richer for those two, since it already reads `overall_assessment`/`decision` when present and only falls back to the one-line placeholder `summary` otherwise.
11. **Consider the Cursor SDK's async client** for the four diff-reviewers once more than one is real, so they can run concurrently instead of sequentially (see the §7 TODO) — this is now a much simpler change than it would have been under the Sprint 6 bridge design, since there's no subprocess-per-call question anymore.
12. Consider setting `ui/components.py::_DEFAULT_EXPANDED["Security"] = True` now that Security is real, matching Architecture/CAB's expanded-by-default treatment (see §7 TODO — small, UI-only, not done in Sprint 12 since it wasn't in that sprint's stated scope).

**Do not start Cursor SDK / LLM work *inside a reviewer* without explicit user confirmation** — every prior sprint in this project has been explicitly scoped by the user one step at a time. Sprint 6/7 explicitly authorized real SDK logic in the Architecture reviewer, Sprint 9 in the CAB reviewer, Sprint 12 in the Security reviewer; `qa.py` and `operations.py` remain off-limits until a future sprint says otherwise.
...
- **Do not add Cursor SDK, LLM, or other AI logic to `qa.py`, `operations.py`, `orchestrator/`, `ui/`, `models/`, or `config.py`'s existing settings** unless the user explicitly requests it in a given sprint. This has been an explicit, repeated constraint across every sprint so far. (`verify_cursor_sdk.py` (Sprint 5), `reviewers/architecture.py` + the `CURSOR_API_KEY`/`CURSOR_SDK_MODEL` settings in `config.py` (Sprint 6/7), `reviewers/cab.py` (Sprint 9), and `reviewers/security.py` (Sprint 12) are the explicit, scoped exceptions so far — `architecture.py`, `security.py`, and `cab.py` are the only three reviewers with real SDK logic.)
