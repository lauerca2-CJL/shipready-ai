...
- `security.py`, `qa.py`, `operations.py`, `cab.py` — `review()` bodies still return **hardcoded** results; untouched by Sprints 6–8.
...
- `prompts/security.md`, `qa.md`, `operations.md`, `cab.md` are still empty template placeholders (`architecture.md` is real as of Sprint 6, expanded in Sprint 8).
- `orchestrator/pipeline.py` still only forwards `ReviewResult.summary` (a plain string) to the dashboard, not the full object — Sprint 8's rich rendering works around this by rendering the whole report into that one string (see §2 / the Sprint 8 row in §4). A future sprint enriching the orchestrator's yield to pass the full `ReviewResult` would let `ui/` build the report layout itself instead of receiving pre-rendered markdown — noted as a real design trade-off, not an oversight.
...
- No *pipeline-level* partial-failure policy yet: `reviewers/architecture.py` now handles its own failures internally (collapsing to `verdict="block"`/`decision="BLOCK"`), but there's no orchestrator-level policy for e.g. "should CAB be told a reviewer's result came from a real failure vs. a real 'block' verdict vs. a placeholder?" — see the §7 TODO.
- `Finding` objects are populated for the Architecture reviewer as of Sprint 8; still always `[]` for `security.py`/`qa.py`/`operations.py`.
...
- Give `security.py`/`qa.py`/`operations.py` the same `decision`/`confidence`/`recommendations`/`overall_assessment` treatment once they get real SDK calls, so all four diff-reviewer cards render consistently.
- Refresh `README.md` and `PRODUCT_VISION.md` — see staleness note below.
- `ruff check .` now reports ~37 pre-existing `UP006`/`UP035`/`UP045` findings (`typing.Optional`/`List`/`Dict`/`Tuple` → `X | None`/`list`/`dict`) across most files, including some Sprint 8 touched — ruff started flagging these once `.python-version` (Sprint 7) gave it a Python 3.12 target to infer. This is pre-existing style debt, not a Sprint 8 regression (see §5 item 8) — a future sprint could modernize it repo-wide in one pass if asked, rather than doing it piecemeal per-file.
...
into a structured, enterprise-report-style output. Remaining candidates:
...
6. **Repeat the pattern for Security, QA, Operations** (each is now a near-copy of `reviewers/architecture.py`'s direct-call shape *and* its Sprint 8 structured-output/rendering shape, with its own `prompts/*.md`), and/or **give `cab.py` the same treatment** once there's a real policy for what CAB should do when some inputs are real analysis and others are still hardcoded (or a real reviewer degraded to `verdict="block"`/`decision="BLOCK"` on an SDK failure — see the §7 partial-failure TODO).
7. **Consider the Cursor SDK's async client** for the four diff-reviewers once more than one is real, so they can run concurrently instead of sequentially (see the §7 TODO) — this is now a much simpler change than it would have been under the Sprint 6 bridge design, since there's no subprocess-per-call question anymore.
8. **Consider enriching `orchestrator/pipeline.py`'s yield** to pass the full `ReviewResult` (not just `.summary`) to the dashboard, once more than one reviewer has Sprint 8-style structured output — that would let `ui/` build each report's layout directly instead of every reviewer having to pre-render its own markdown. Sprint 8 deliberately didn't do this since the orchestrator was explicitly out of scope; confirm with the user before touching it.
...
- **`reviewers/architecture.py::review()` must never raise.** Every SDK-call/parsing failure mode must be caught and converted to a valid `ReviewResult` (currently `verdict="block"`/`decision="BLOCK"`) — `orchestrator/pipeline.py` has no try/except around reviewer calls, so an uncaught exception here would crash the whole dashboard run. Preserve this if you touch the function again — including `_blocked_result()`, which must keep rendering through `_render_report_markdown()` so failure output stays visually consistent with success output.
- **`orchestrator/pipeline.py` only forwards `ReviewResult.summary`** (a plain string) to the dashboard — not `decision`/`confidence`/`findings`/`recommendations`/`overall_assessment`. This is why Sprint 8 renders the whole report *into* `summary` as one markdown string rather than changing the orchestrator's yield. Don't assume the dashboard can read those other fields directly; it can't, without an orchestrator change (see the §8 candidate for that).
